...
check_port() {
  port="$1"
  R=$(MSYS_NO_PATHCONV=1 curl -s -G -b "$COOKIE" "$BASE" --data-urlencode "target=127.0.0.1:$port" --data-urlencode "path=/" --max-time 4)
...
seq "$1" "$2" | xargs -P 20 -I{} bash -c 'check_port "$@"' _ {}
